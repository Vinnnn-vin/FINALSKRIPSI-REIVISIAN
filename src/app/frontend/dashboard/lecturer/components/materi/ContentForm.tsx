...
export default function ContentForm({ onAddContent }: ContentFormProps) {
  ...
  const renderPreview = () => {
    if (lessonType === "url" && lessonUrl) {
      // Preview for URL
      const isYoutube = lessonUrl.includes('youtube.com') || lessonUrl.includes('youtu.be');
      ...
      return (
        <Card withBorder p="sm">
          <Text size="sm" fw={500}>Preview URL:</Text>
    ...
    if (uploadState.url) {
      return (
        <Card withBorder p="sm">
          <Text size="sm" fw={500}>File berhasil diupload:</Text>
        ...
        <Group justify="space-between">
          <Stack gap={0}>
            <Text size="sm" c="dimmed">
              Status: {
                lessonType === "url" ? 
                  (lessonUrl.trim() ? "URL siap digunakan" : "Masukkan URL") :
                uploadState.uploading ? "Sedang upload..." :
                uploadState.url ? "File siap digunakan" :
                uploadState.error ? "Upload gagal" :
                lessonFile ? "File dipilih, sedang upload..." : "Pilih file untuk upload"
              }
            </Text>
            {uploadState.error && (
              <Text size="xs" c="red">
          ...
          >
            {uploadState.uploading ? "Uploading..." : "Tambah ke Daftar Konten"}
          </Button>
        </Group>
      </Stack>
    </Card>
